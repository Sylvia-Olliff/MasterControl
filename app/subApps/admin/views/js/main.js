$(document).ready(function() {
	//const PATH_ 		 = "/admin/";
	const BUILD_DEV_PATH = "/www/websmart/htdocs/wsphp/RateMileQuery/Dev/TruckRateBuilder_DEV/";
	const QUERY_DEV_PATH = "/www/websmart/htdocs/wsphp/RateMileQuery/Dev/TruckRateViewer_DEV/";
	const BUILD_PRO_PATH = "/www/websmart/htdocs/wsphp/RateMileQuery/Prod/TruckRateBuilder/";
	const QUERY_PRO_PATH = "/www/websmart/htdocs/wsphp/RateMileQuery/Prod/TruckRateViewer/";
	...
	var socket = io.connect("as400.rogers-brown.com:88/admin/test");
	...
	$('#dev-env').on('click', function(){
		$('#DEV').hide();
		$('#PRO').hide();
		$('#interface').load("admin/view/Dev-Main.html", function(){
			...
			$('#rateBuilderDev').on('click', function(){
				buildSettings("admin/view/Builder-Control-Dev.html", BUILD_DEV_PATH, errorDialog);
			});

			$('#rateQueryDev').on('click', function(){
				querySettings("admin/view/Query-Control-Dev.html", QUERY_DEV_PATH, errorDialog);
			});
		});
	});
	...
	$('#pro-env').on('click', function(){
		$('#DEV').hide();
		$('#PRO').hide();
		$('#interface').load("admin/view/Pro-Main.html", function(){
			$('#btn-back').on('click', function(){
				$('#DEV').show();
				$('#PRO').show();
				$('#interface').html("");
			...
			$('#rateBuilderPro').on('click', function(){
				buildSettings("admin/view/Builder-Control-Pro.html", BUILD_PRO_PATH, errorDialog);
			});

			$('#rateQueryPro').on('click', function(){
				querySettings("admin/view/Query-Control-Pro.html", QUERY_PRO_PATH, errorDialog);
			});
		});
	});
		...
		$.ajax({
			...
			success: function(response) {
				if (response == "SUCCESS") {
					...
					errorDialog.setTitle("ERROR!");
					errorDialog.setType(BootstrapDialog.TYPE_WARNING);
					errorDialog.setMessage("<p>Something went wrong behind the scenes, check the info.log file for more information</p>");
					errorDialog.open();
				} else {
